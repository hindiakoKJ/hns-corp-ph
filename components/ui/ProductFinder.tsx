...
// which world each product belongs to
const CATEGORY: Record<string, Exclude<Cat, 'all'>> = {
  ...
  Everafter: 'life', Scatto: 'life', CVAssist: 'life', LOCATR: 'life', Steady: 'life',
};

const DOORS: { id: Cat; k: string; d: string }[] = [
  { id: 'all',        k: 'Everything', d: 'All ten products' },
...
export default function ProductFinder() {
  ...
  return (
    <>
      <div className="st-doors" role="tablist" aria-label="Find your product">
        {DOORS.map((d) => (
          <button
            key={d.id}
            role="tab"
            aria-selected={cat === d.id}
            className={`st-door ${cat === d.id ? 'active' : ''}`}
            onClick={() => setCat(d.id)}
          >
      ...
      <div className="st-grid">
        {shown.map((p) => (
          <Link key={p.key} href={p.href} className="st-card" style={{ ['--accent' as string]: p.accent }}>
            <div className="top">
              <span className="logo"><img src={p.logo} alt={p.name} /></span>
              <span className={`st-status ${p.status}`}>{p.status === 'live' ? 'Live' : 'Soon'}</span>
            </div>
