...
type Status = 'live' | 'upcoming' | 'future';

interface Product {
  key: string;
  Mark?: ComponentType<LogoProps>;
  name?: string;
  /** Tailwind text color class for the mark, e.g. "text-clerque-600" */
  colorClass?: string;
  /** Tailwind bg color class for the mark tile, e.g. "bg-clerque-600/10" */
  tintClass?: string;
  kind: string;
  status: Status;
}
    ...
    <section className="bg-gradient-to-b from-paper to-slate-50 px-8 py-16 md:px-16 md:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-7 flex items-center gap-6">
          <div className="flex h-24 w-24 items-center justify-center rounded-[22px] bg-gradient-to-br from-hns-600 to-hns-deep shadow-[0_14px_40px_-10px_rgba(8,145,178,0.4)]">
            <HNSLogo size={60} color="#FFFFFF" />
          </div>
          <div>
            <h1 className="text-4xl font-semibold tracking-wordmark-tight text-ink md:text-5xl">
              HNScorpPH
            </h1>
...
function ProductCard({ product }: { product: Product }) {
  if (product.status === 'future') {
    return (
      <div className="flex min-h-[168px] flex-col gap-3 rounded-xl border-[1.5px] border-dashed border-ink/15 p-5">
        ...
        <div>
          <div className="text-base font-semibold tracking-tight text-ink/35">
            Future product
          </div>
          <div className="mt-1 text-[11px] leading-snug text-ink/50">
            {product.kind}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="relative flex min-h-[168px] flex-col gap-3 rounded-xl border border-ink/10 bg-paper p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
      ...
      <div className={`flex h-11 w-11 items-center justify-center rounded-[10px] ${product.tintClass}`}>
        <Mark size={28} className={product.colorClass} />
      </div>
